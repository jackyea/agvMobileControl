...
{
	/*Timer到期后执行回调函数timer_notify(),
	sigev.sigev_notify_function = timer_notify 这个就是指定回调函数，，进入锁后调用*/
...
void TimerInit(void)
{
	...
	sigev.sigev_notify = SIGEV_THREAD; ////定时器到期后内核创建一个线程执行sigev_notify_function函数,sigev_notify_function=timer_notify
	sigev.sigev_notify_attributes = NULL;
	sigev.sigev_notify_function = timer_notify; ///timer_notify这个就是指定回调函数,以便执行time dispatch
...
void StartTimerLoop(TimerCallback_t init_callback) ///StartTimerLoop(&Init);  init_callback=Init:init node state for master
{
	EnterMutex();
	// At first, TimeDispatch will call init_callback.
	SetAlarm(NULL, 0, init_callback, 0, 0);  ///write Init() into s_timer_entry,and wait for triging by time dispatch()
	LeaveMutex();
}
...
TIMEVAL getElapsedTime(void) //距离上次执行time dispatch 时间差
{
	struct timeval p;
