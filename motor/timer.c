...
TIMER_HANDLE SetAlarm(CO_Data* d, UNS32 id, TimerCallback_t callback, TIMEVAL value, TIMEVAL period)
{
	...
	for(row_number=0, row=timers; row_number <= last_timer_raw + 1 && row_number < MAX_NB_TIMER; row_number++, row++)
	{
		...
		{	/* just store */
			...
			row->interval = period;
			row->state = TIMER_ARMED;//active此函数
			return row_number;
		}
	}
...
void TimeDispatch(void)
{
	...
	{
		if (row->state & TIMER_ARMED) /* if row is active 如果函数时激活状态*/
		{
			if (row->val <= real_total_sleep_time) /* to be trigged */
			{
				if (!row->interval) /* if simply outdated 非周期执行的函数 */
				{
				...
				else /* or period have expired */
				{
					...
					if(row->val < next_wakeup)//如果此程序下次启动时间在下次time dispatch执行之前，则修改下次time dispatch 唤醒时间
						next_wakeup = row->val;
				}
			}
			else
	...
	setTimer(next_wakeup);  //重新给timer设定时间,到期后执行time dispatch

	/* Then trig them or not. */
	for(i=0, row = timers; i<=last_timer_raw; i++, row++)
	{
		if (row->state & TIMER_TRIG) /*if trig row->state=0000 0010(bit)=2=UNS 8, TIMER_TRIG=2=10(bit)*/
		{
			row->state &= ~TIMER_TRIG; /* 重置状态标识符为0，既不是激活状态TIMER_ARMED 也不是触发状态 TIMER_TRIG
										reset trig state (will be free if not periodic) ~TIMER_TRIG: invert 10 to 01,reset state*/
			if(row->callback)
